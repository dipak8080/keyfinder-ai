...
#
# 60 per 5 minutes (was 30, before that 10) so a 50-file batch from the key
# finder's batch mode fits. The batch queue sends two files at a time, so one
# IP holds at most two of the MAX_CONCURRENT_ANALYSIS slots.
ANALYZE_RATE_LIMIT_MAX_REQUESTS = int(os.environ.get("ANALYZE_RATE_LIMIT_MAX_REQUESTS", "60"))
...
SEPARATION_SHARED_DAILY_MAX_REQUESTS = int(os.environ.get("SEPARATION_SHARED_DAILY_MAX_REQUESTS", "15"))
...
RESAMPLE_ALLOWED_RATES = (8000, 16000, 22050, 44100, 48000, 96000)
...
SILENCE_SPLIT_MIN_SEGMENT_MAX_SECONDS = float(os.environ.get("SILENCE_SPLIT_MIN_SEGMENT_MAX_SECONDS", "600"))
# When the detected count exceeds SILENCE_SPLIT_MAX_SEGMENTS, merge across
# the shortest gaps down to the limit instead of failing the job.
SILENCE_SPLIT_AUTO_MERGE = os.environ.get("SILENCE_SPLIT_AUTO_MERGE", "1") == "1"
